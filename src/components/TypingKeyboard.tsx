...
export interface TypingKeyboardProps extends React.HTMLAttributes<HTMLDivElement> {
  /** Array of texts to auto-type in a loop */
  messages?: string[];
...
export function TypingKeyboard({
  ...
  useEffect(() => {
    ...
    if (!kb || !screen || !messages.length) return;
    ...
    const pressKey = (kc: number) => {
      ...
      if (el) {
        ...
        setTimeout(() => {
          if (alive && el) el.classList.remove("tk-key--down");
        }, 80);
      }
    };
    ...
    const typeNext = () => {
      ...
      if (charIdx >= currentText.length) {
        // Pause at the end of the message, then clear and start typing next
        timer = setTimeout(() => {
          if (!alive || !screen) return;
  ...
  return (
    <div className={`tk-container ${className || ""}`} {...props}>
      <style>{`
        ...
        /* Keyboard body - Dark Theme Casing */
        .tk-keyboard {
          width: 500px; height: 160px;
          transform: perspective(10000px) rotateX(50deg) rotateZ(-25deg);
        }
        .tk-keyboard__front { width: 500px; height: 25px; transform: rotateX(-90deg) translateZ(80px); background-color: #222222; border-top: 1px solid rgba(255, 255, 255, 0.05); }
        .tk-keyboard__back  { width: 500px; height: 25px; transform: rotateX(90deg) translateZ(80px); background-color: #111111; }
        .tk-keyboard__top   {
          display: flex; flex-direction: column; justify-content: space-around;
          width: 500px; height: 160px;
          transform: rotateY(0deg) translateZ(12.5px);
          background-image: linear-gradient(to bottom, color-mix(in srgb, ${accentColor} 20%, #1a1a1a), #151515);
        }
        .tk-keyboard__bottom {
          width: 500px; height: 160px;
        ...
        .tk-keyboard__right { width: 25px; height: 160px; transform: rotateY(90deg) translateZ(250px); background-color: #1c1c1c; }
        .tk-keyboard__left  { width: 25px; height: 160px; transform: rotateY(90deg) translateZ(-250px); background-color: #111111; }

        /* Screen - Glowing Console Terminal */
        .tk-screen {
          ...
          box-shadow:
            ...
            0 0 20px color-mix(in srgb, ${accentColor} 20%, transparent);
        ...
        /* Individual key - Dark Keycaps */
        .tk-key { width: 30px; height: 27px; transition: .05s ease; }
        .tk-key--w2 { width: 60px; }
        .tk-key--w3 { width: 90px; }
        .tk-key--w6 { width: 195px; }

        .tk-key__front { width: 30px; height: 8px; transform: rotateX(-90deg) translateZ(13.5px); background-color: #1b1b1b; }
        .tk-key__front--w2 { width: 60px; }
        .tk-key__front--w3 { width: 90px; }
        .tk-key__front--w6 { width: 195px; }

        .tk-key__back { width: 30px; height: 8px; transform: rotateX(90deg) translateZ(13.5px); background-color: #111111; }
        ...
        .tk-key__top {
          ...
          box-shadow: inset 0 0 0 0.5px rgba(255,255,255,0.06), 0 -1px 2px rgba(0,0,0,0.5);
        }
        .tk-key__top--w2 { width: 60px; }
        .tk-key__top--w3 { width: 90px; }
        .tk-key__top--w6 { width: 195px; }

        .tk-key__bottom { width: 30px; height: 27px; transform: rotateY(180deg) translateZ(4px); background-color: #0b0b0b; }
        .tk-key__bottom--w2 { width: 60px; }
        .tk-key__bottom--w3 { width: 90px; }
        .tk-key__bottom--w6 { width: 195px; }

        .tk-key__right { width: 8px; height: 27px; transform: rotateY(90deg) translateZ(15px); background-color: #151515; }
        ...
        .tk-key__left {
          ...
          background-image: linear-gradient(to bottom, #1f1f1f, #151515);
        }
